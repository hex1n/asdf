...
**Landing contract (plan-landing tasks):** when landing an accepted plan, restate two things before the first edit — the envelope (the target repo/worktree plus files, tables, interfaces) and the completion criterion. If no machine-checkable completion criterion exists (a test command, SQL assertion, or expected response), ask for one before editing instead of inventing a soft one; for a trivial single-file change with no data or interface impact, a one-line current-vs-expected statement suffices. If mid-landing the work requires touching anything outside the restated envelope, stop and confirm before proceeding — do not silently widen scope. When the user has authorized a loop (iterate edit → verify → fix until the criterion passes), iterate without prompting between rounds; stop and report when the criterion passes, when it is proven unreachable, when the same failure repeats twice, or after eight rounds when the user has stated no cap.
