...
- Local task-state directories such as `.taskloop/` only to understand active handoff state; do not promote their contents into durable docs.
...
- Local task state: active goals, `.taskloop/task.json`, checkpoints, probes, and temporary loop state. It is gitignored, non-authoritative, and never overrides the current user message or global contract.
...
- Local `.taskloop/` state is ignored by git, contains only run-time state such as `task.json`, is managed with the taskloop CLI when available, and is not cited as durable project policy or business verification evidence.
...
First read:
  <skill>/SKILL.md
  <skill>/REFERENCE.md
Then inventory only the project's root agent instructions, direction/context docs, repo profiles, workflow directories, goal/evidence templates, verification menus, and any local `.taskloop/` handoff state.
