...
const CSS = `
:root{
--page:#f9f9f7;--card:#fcfcfb;--line:#e6e5e0;--hair:#efeee9;
--ink:#0b0b0b;--ink2:#52514e;--muted:#898781;--accent:#1f5f8b;
...
h1{font-size:24px;line-height:1.4;margin:0 0 6px;letter-spacing:-.01em}
h2{font-size:19px;margin:52px 0 16px;padding-bottom:8px;border-bottom:1px solid var(--line);letter-spacing:.01em}
h3{font-size:16px;margin:30px 0 10px}h4{font-size:14.5px;margin:20px 0 6px;color:var(--ink2)}
p,li,td,th,dt,dd,a,h1,h2,h3,h4{overflow-wrap:anywhere;word-break:break-word}
code{background:#f1f0ec;border-radius:3px;padding:.1em .34em;color:var(--ink2);
 font-family:"Cascadia Mono",Consolas,"Courier New",monospace;font-size:.86em;overflow-wrap:anywhere}
pre{background:#1a1a19;color:#e8e7e0;border-radius:6px;padding:14px 16px;overflow-x:auto;font-size:12.5px;line-height:1.6}
pre code{background:none;color:inherit;padding:0}
a{color:var(--accent);text-underline-offset:2px}
a:focus-visible,summary:focus-visible{outline:2px solid var(--accent);outline-offset:3px}
.sub{color:var(--muted);margin:0 0 20px;font-size:13px;line-height:1.65}

/* 卡片：发丝边、留白优先，不用重块 */
.card{background:var(--card);border:1px solid var(--line);border-radius:8px;padding:16px 20px;margin:0 0 12px}
.lbl{font-size:11px;letter-spacing:.1em;color:var(--muted);text-transform:uppercase;margin:0 0 11px;font-weight:600}

/* 英雄数字：每视图一个，墨色 + 旁置色标承载状态，比例数字 */
.hero{display:flex;align-items:baseline;gap:14px;margin:0 0 4px}
.hero .mark{width:10px;height:34px;border-radius:2px;flex:0 0 auto;align-self:center}
.hero .fig{font-size:46px;font-weight:600;line-height:1;letter-spacing:-.02em;color:var(--ink)}
.hero .cap{font-size:15px;color:var(--ink2)}
.outcome .lead{margin:0;font-size:18px;line-height:1.65;color:var(--ink)}

/* KPI 行：stat tile，零值退到背景，非零带色标 */
.verdicts{display:grid;grid-template-columns:repeat(5,1fr);gap:1px;background:var(--line);
 border:1px solid var(--line);border-radius:6px;overflow:hidden;margin-top:14px}
.v{background:var(--card);padding:12px 14px}
.v .n{display:block;font-size:22px;font-weight:600;line-height:1.15;color:var(--ink)}
.v .k{display:flex;align-items:center;gap:6px;font-size:12px;color:var(--ink2);margin-top:5px}
.v .k i{width:8px;height:8px;border-radius:2px;flex:0 0 auto;font-style:normal}
...
.v.zero{background:#f7f6f2}.v.zero .n,.v.zero .k{color:var(--muted)}
.v.zero .k i{background:var(--neutral)!important;opacity:.45}
.headline{font-size:14px;margin:14px 0 0;line-height:1.7;color:var(--ink2)}

/* 信任条：键值对，不做成卡片 */
.trust{display:grid;grid-template-columns:repeat(4,1fr);gap:0;font-size:12.5px;
 border-top:1px solid var(--hair)}
.trust div{padding:12px 16px 2px 0;border-right:1px solid var(--hair)}
.trust div:last-child{border-right:none}
.trust .t{display:block;font-size:11px;letter-spacing:.06em;text-transform:uppercase;color:var(--muted);margin-bottom:5px}
/* 步进图：细连接符、发丝边，不做成一排色块 */
.flow{display:flex;flex-wrap:nowrap;align-items:stretch;gap:0}
.node{flex:1 1 0;min-width:0}
.node a{display:block;height:100%;text-decoration:none;color:inherit;padding:2px 12px 2px 0}
...
.nplain{display:block;font-size:13.5px;font-weight:600;line-height:1.4;color:var(--ink)}
.ntech{display:block;font-size:11.5px;color:var(--muted);margin-top:4px;line-height:1.4}
.arrow{align-self:flex-start;color:var(--muted);font-size:14px;flex:0 0 auto;padding:1px 10px 0 0}
.sideflows{margin-top:14px;padding-top:12px;border-top:1px solid var(--hair);font-size:12.5px;color:var(--muted);
...
.two{display:grid;grid-template-columns:1fr 1fr;gap:14px;align-items:start}.two .card{margin:0 0 14px}
.three{display:grid;grid-template-columns:1fr 1fr 1fr;gap:14px;align-items:start}.three .card{margin:0}
ul.bare{margin:0;padding:0}
.dec li{margin:0 0 8px;list-style:none;font-size:12.5px;line-height:1.5;color:var(--ink2)}
.dec li .hd{display:flex;align-items:center;gap:8px;margin-bottom:1px}
.dec li a{font-weight:600}
.risk li{margin:0 0 8px;list-style:none;font-size:13px;line-height:1.6;color:var(--ink2);
...
ol.slice li{margin:0 0 9px;font-size:13px;line-height:1.5;counter-increment:s;padding-left:24px;position:relative}
ol.slice li::before{content:counter(s);position:absolute;left:0;top:0;width:17px;height:17px;border-radius:50%;
 background:#f1f0ec;color:var(--ink2);font-size:11px;font-weight:600;text-align:center;line-height:17px}
...
ol.rerun li{margin:0 0 11px;counter-increment:r;padding-left:26px;position:relative;line-height:1.6}
ol.rerun li::before{content:counter(r);position:absolute;left:0;top:1px;width:18px;height:18px;border-radius:50%;
 background:#f1f0ec;color:var(--ink2);font-size:11px;font-weight:600;text-align:center;line-height:18px}
table{border-collapse:collapse;width:100%;font-size:13.5px}
.tw{overflow-x:auto;background:var(--card);border:1px solid var(--line);border-radius:8px;margin:0 0 14px}
th,td{border-bottom:1px solid var(--hair);padding:9px 13px;text-align:left;vertical-align:top;line-height:1.65}
thead th{background:transparent;font-size:11px;letter-spacing:.07em;text-transform:uppercase;
 color:var(--muted);font-weight:600;border-bottom:1px solid var(--line);padding-top:14px}
tbody tr:last-child td{border-bottom:none}
td:first-child{font-variant-numeric:tabular-nums}

/* 状态/处置一律「小色标 + 图标 + 文字」，颜色从不单独承载含义 */
.badge{display:inline-flex;align-items:center;gap:5px;font-size:11.5px;font-weight:600;color:var(--ink2);white-space:nowrap}
.badge::before{content:"";width:8px;height:8px;border-radius:2px;flex:0 0 auto;background:var(--neutral)}
...
.b-oracle{color:var(--muted);font-weight:500}.b-oracle::before{display:none}
.disp{display:inline-flex;align-items:center;gap:5px;font-size:11.5px;font-weight:600;color:var(--ink2);white-space:nowrap}
.disp::before{content:"";width:8px;height:8px;border-radius:2px;flex:0 0 auto;background:var(--neutral)}
...
#tree .branchrow th,#exec .branchrow th{background:#e4ebf2;font-size:15px;color:#123c58;border-bottom:2px solid var(--accent)}
.scid{font-weight:700;text-decoration:none;font-size:15px}
.sctitle{margin-top:4px;font-weight:600;line-height:1.4}
.pri{display:inline-flex;align-items:center;gap:5px;font-size:11px;font-weight:600;color:var(--ink2);margin-left:10px}
.pri::before{content:"";width:7px;height:7px;border-radius:2px;flex:0 0 auto;background:var(--neutral)}
...
@media (max-width:1000px){
 ...
 table.stack td{display:block;width:auto;border-bottom:none}
...
<th style="width:96px">ID</th><th>诊断</th><th style="width:172px">处置</th><th style="width:180px">影响场景</th><th style="width:64px">详情</th>
</tr></thead><tbody>
...
const page = (title, sub, body) => `<!DOCTYPE html>
<html lang="zh-CN"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width, initial-scale=1">
<title>${esc(title)}</title><style>${CSS}</style></head>
<body><div class="wrap"><h1>${esc(title)}</h1>${sub ? `<p class="sub">${sub}</p>` : ''}${body}</div></body></html>`;
...
function render(data, dir) {
    ...
    fs.writeFileSync(path.join(dir, data.out), page(data.title, data.subtitle, body), 'utf8');
...
for (const k of ['mode', 'title', 'canonical', 'out', 'scenarios']) {
