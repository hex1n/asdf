...
两三行源码合并成一个片段，在记录里用空格连接、写在同一物理行），而不是往里缩成
`.accepts(value)) {` 这种唯一却读不出意思的碎片。
**超过两个锚点时，实现理由要点名每个站点做什么**，用缩进 text 块列出文件与角色即可。
导出不同后果的锚点属于**不同的记录**——不同后果就是不同标题，那一问同时是拆分判据。

同一个后果下的锚点数不是"大小"，而是**有多少地方必须各自记住这条规则**：一个锚点通常意味着理由在
代码里根本无法表达（选定的常量、舍入方向、名字与类型都暗示不出的顺序），记录就是它唯一的去处；多个
锚点意味着规则靠重复执行，任一处单独改掉就破——这种记录已经把每个站点点名了，等于一份划好范围的
重构待办，等规则有了唯一 owner，记录会塌缩成一个锚点或不再需要。也有收敛不了的：协议的两端、强行
共享会造出更坏依赖的两层、框架要求每个实现各自满足的契约——那里记录本身就是 owner，不算欠债。
文件路径和代码片段字段各占一个物理行；只有实现理由支持缩进两格的续行。
...
W-ID 是名字不是位置：它按记录锚住的东西命名（`W-调用顺序`），在仓库内唯一且不分大小写，
与前后记录的编号没有关系，数值大也不代表更新。标题行写作 `## W-名字 · 标题`，间隔号前要有空格；
名字本身可以含 `·`。标题陈述这次的判断，随理解变化可以改写，
