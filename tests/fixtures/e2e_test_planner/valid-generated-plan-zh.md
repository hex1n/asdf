...
- 覆盖：主链路 J1→J2→J3（订单 → 支付 → 发票），1 个 P0 场景；核心切片＝订单-E2E-001。
- 主风险：重复支付回调导致重复开票（幂等）。
- 未决：支付网关超时 SLA 无来源证据 `OUT-OF-SCOPE`（首切片范围外）。
...
- 索引：节点 `N1` ｜ 优先级 P0 ｜ 副作用类型 `additive-retained` ｜ 就绪门禁 → §10 入口（支付替身可用）。调度详情见 §6 DAG。
- 目的：覆盖订单到支付再到发票的依赖链。
- 优先级：P0。
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| N1 | 订单-E2E-001 | J1-J3、支付替身可用 | 买家账号、商品、`orderId` | `orderId`、`paymentEventId`、`invoiceId` | API、DB、任务、替身 | 订单表、支付事件表、发票表 | `orderId` 批次前缀 | unsafe：主链路按产出 ID 串行消费 | 发票探针完成后按 `orderId` 清理 | none |
...
- 支付网关超时和撤销不在首个切片内 `OUT-OF-SCOPE`（首切片范围外）。
