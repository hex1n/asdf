...
For any trigger surface, prefer a project-declared execution adapter, test harness, CLI wrapper, browser driver, queue/job tool, callback harness, or other safe command over hand-written mechanics. This executor owns scenario selection, environment gates, data policy, evidence capture, diagnosis, cleanup, and reporting; the adapter or tool owns surface-specific target or locator resolution, capability discovery, safe describe/dry-run behavior, request or action encoding, metadata/options, invocation, and replay mechanics. If no safe adapter, tool, or project-declared command exists for the required trigger, mark the scenario `BLOCKED-BY-TOOLING` with the missing capability named. See [REFERENCE.md](REFERENCE.md#execution-adapter-boundary).
...
`Scenario Results` is self-contained per row, so the verdict and why are readable from the row without joining across sections: beside the terminal status, each row carries the **expected** and **actual** outcome (one-line deltas, depth deferred to the evidence link), a **diagnosis** token (the §5 classification only — e.g. `product`/`environment` — not the reason prose), and direct links to its evidence anchor and preserved scene. The full failure reason and disposition stay single-sourced in `Failures / Defects / Plan Gaps`, which is keyed by defect/root-cause and which the row links to; a defect that spans several scenarios is written there once with an affected-scenario list, never restated per row. `Evidence Index` is organized **per scenario** as a short proof chain — probe → expected → actual → raw-artifact paths — which is exactly where each row's evidence link lands, so following a link reaches the proof, not an undifferentiated dump. See [REFERENCE.md](REFERENCE.md#scenario-results--evidence-legibility).
...
After writing `execution-report.md`, you can self-check its protocol shape: `python3 <e2e-test-executor>/scripts/check_report.py <report-or-run-dir> --strict` flags a missing required section, an illegal scenario status, a `failed` row with no preserved scene, a diagnosis that is not a closed-set token, or a Next-Actions item that is not `OPEN`. It validates report self-consistency only, never truthfulness — a structurally clean report can still be untruthful — so it complements, never replaces, evidence review.
