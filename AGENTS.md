...
Before changing any skill, load and apply `writing-for-agents` (a user-level skill outside this repo) as the authoring frame for predictability, information hierarchy, context pointers, leading words, progressive disclosure, and pruning. Read its `SKILL.md`; when the change affects a skill's invocation or frontmatter, also read its `SKILL-MECHANICS.md`. Only when `writing-for-agents` cannot be loaded from disk, apply the authoring criteria listed above directly and record the fallback in the round notes before editing.
