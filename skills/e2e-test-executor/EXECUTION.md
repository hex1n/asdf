...
If a required real dependency is unreachable, use its already-declared double. When no double exists, capture endpoint, error, timestamp, and retry evidence; mark the scenario `blocked`; classify the root cause as `environment defect` with `BLOCKED-BY-ENVIRONMENT`. It becomes a product defect only when product code ran and violated an established expectation. The only exception is a scenario whose declared purpose and `Automation` level explicitly cover dependency-down, timeout, or recovery behavior: the outage is then its expected input, and it passes only when the specified fallback, retry, compensation, or error contract is observed.
...
When creating or mutating data, decide cleanup before the write and later emit runnable seed and cleanup scripts when the surface is scriptable. A generated helper that performs this run's mutations is hashed before its first mutation and the hash recorded, under the helper rules in [REPORTING.md](REPORTING.md#keep-one-canonical-proof-path). For an absent target, canonicalize the existing parent and authorized boundary first, reject symlink/traversal components, validate the proposed basename and direct-child relation lexically, and only then create it; canonicalize the result immediately and re-prove containment. Creating a target and checking it afterward is too late. A relative path, system temporary directory, or tool default is not implicitly authorized. Record the validated parent and proposed target in the ledger, then write. If a target is outside the boundary, stop before the first mutation and choose an in-boundary fixture or mark the path blocked. Under preserve-traces policy, keep self-owned diagnostic state with owner, TTL, cleanup command, and risk.
