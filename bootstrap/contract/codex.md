...
- 自驱：用户授权一次（"循环到 X 为止"）后用 /loop <判据> 作驱动器自驱推进，把用户从人肉时钟撤出——驱动器（/goal 每轮复查判据续跑，或 ralph-loop 重喂 fresh context）负责自驱，判据闸门负责机器兜底判停，判据本身负责终止；三者合起来才闭合。中途只在真阻塞、不可逆/高风险决策、越界或判停兜底时回来。
