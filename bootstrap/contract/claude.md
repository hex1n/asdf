...
1. **触发**：先做最小必要核实。接到落地/排查类任务，确定机器可查的完成判据（测试命令、SQL 断言、
   ...
   "现状 → 期望"代替正式判据。
2. **收敛**：方案类任务先不写码（建议直接用 Plan Mode 只读模式承载）；`/converge`
   只用于显式要求或高风险未收敛场景。用户已明确拍板（如"按方案1来/落地/开始实现"）
   后，不自行回到收敛，除非出现新的硬阻塞或用户要求再证伪。
3. **落地**：拍板后按 `/land` 契约循环——复述 Touch 清单 + 判据 → 改 → 验 → 审 → 修，
   判据满足才停；需要触碰清单外目标时立即停下确认。用户拍板是执行许可，不用通用流程
   覆盖用户判断。仓库已有 `.agent-workflows/` 时，把当前 Touch 清单同步到
   `.agent-workflows/touch-list.json`，让 hook 记录/拦截写目标；该目录是 gitignored
   的 agent 私有运行状态，不是项目政策源。并行推进多个循环时各用独立 git worktree，
   不共享工作目录。
...
7. **事实校验**：声称"已读/已写/已完成/已验证"必须有真实工具结果或只读复核支撑；
   工具失败、未调用或输出不匹配时，必须报告失败，不能用流程文本替代事实。
8. **元循环**：元循环是离线维护任务；按需或每月重跑 session 分析（analyze-sessions.py）
...
- **Touch 清单**：落地前复述目标仓库/工作目录与将要触碰的文件/表/接口清单；执行中
  需要触碰清单外目标时，立即停下确认，不得顺手扩界。若仓库已有 `.agent-workflows/`，
  同步维护 `.agent-workflows/touch-list.json`；`.agent-workflows/evidence-ledger.jsonl`
  只作本地 hook 范围/过程证据，不能替代测试、SQL、API 响应或 diff 等业务验证证据。
