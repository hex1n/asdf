...
- Local run-state directories such as `.agent-workflows/` only to understand active handoff state; do not promote their contents into durable docs.
...
- Local run state: active goals, `touch-list.json`, `evidence-ledger.jsonl`, checkpoints, probes, and temporary loop state. It is gitignored, non-authoritative, and never overrides the current user message or global runtime contract.
...
- Local `.agent-workflows/` state is ignored by git, contains only run-time state such as `touch-list.json` and `evidence-ledger.jsonl`, is managed with the workflow hook CLI when available, and is not cited as durable project policy or business verification evidence.
...
First read:
  <skill>/SKILL.md
  <skill>/REFERENCE.md
Then inventory only the project's root agent instructions, direction/context docs, repo profiles, workflow directories, goal/evidence templates, verification menus, and any local `.agent-workflows/` handoff state.
