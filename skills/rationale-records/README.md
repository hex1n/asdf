...
一个理由涉及多处代码时，可以重复“文件路径／代码片段”对：**锚点锚在"破坏性改写会落到的地方"**——
把标题那一问套到每个锚点上，按显而易见的方式改掉这行，标题说的后果会不会发生？公式、常量、选定的
类型、顺序、落库写入都算；只是把值传给下一个调用的行不算，改它什么也不会发生。片段是**引用代码本身**，定位只是引用的
副作用（所以用文本而不是会随编辑漂移的行号）：取完整的判断或语句，不够唯一时**往外扩**（匹配忽略换行，
两三行也是一个片段），而不是往里缩成 `.accepts(value)) {` 这种唯一却读不出意思的碎片。
**超过两个锚点时，实现理由要点名每个站点做什么**，用缩进 text 块列出文件与角色即可。字段下方缩进两格的行属于该字段；
