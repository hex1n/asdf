...
const ACTIONS = [];
// Matches the loop hook command this installer writes, so the Claude and Codex
// hook editors prune the existing block before re-adding the current one
// (idempotent distribution). It must track the live hook command name
// (taskloop.mjs); if it lags a rename, each re-run stacks another duplicate
// hook instead of replacing it. v1 is deleted, so this matches only the v2
// command, with no legacy names. tests/bootstrap_install.test.mjs locks this.
export const LOOP_HOOK_RE = /taskloop\.mjs/i;
...
export const STOP_HOOK_TIMEOUT_SECONDS = 330;
export const PRETOOL_HOOK_TIMEOUT_SECONDS = 30;
...
export function pruneWorkflowHookGroups(groups) {
...
export function addClaudeHook(settings, event, matcher, command, timeoutSeconds) {
  settings.hooks ??= {};
...
function stripManagedHookBlock(text) {
  let next = text;
  for (;;) {
    const start = next.indexOf(HOOK_BEGIN);
    if (start < 0) return next;
    const end = next.indexOf(HOOK_END, start + HOOK_BEGIN.length);
    if (end < 0) return next;
    next = next.slice(0, start).trimEnd() + "\n" + next.slice(end + HOOK_END.length).replace(/^\s+/, "");
  }
}
...
function jsonConfigProblem(target) {
...
// Self-execute only when run as the CLI entrypoint; when imported by a test
// (tests/bootstrap_install.test.mjs) the top-level stays side-effect-free so
// the exported helpers can be unit-tested without running a real install.
