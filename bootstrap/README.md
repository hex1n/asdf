...
- **判据闸门是协作式兜底，不是对抗性防线**：Stop hook 执行 `criterion` 防的是
  ...
  也不防篡改。
- **机器口径与真实担保**：预算里机器执行的"迭代"是**连续被拦的停止尝试**（判据
  通过即清零），不是"改+验一轮"——埋头改、从不尝试停的循环不消耗闸门预算，由
  可选的写/墙钟预算（`init --max-writes` / `--max-wall-clock-minutes`，默认关，
  只拦真写、读与验证永不拦）兜底。在下一条 fail-open 降级之下，整套机制给出的
  硬担保只有一条且刻意收窄：**strict 模式、环境健康、agent 不直接改运行态文件时，
  红判据不会以 `success` 收口**（闸门与 `close` 两条写 success 的路径都复跑判据）。
  它精确覆盖"看着像完成了"这一最高频失效，不承诺更多。
